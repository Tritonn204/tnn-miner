...
public:
  ...
  __device__ bool add(const void* input, uint64_t length)
  {
    // no data ?
    if (!input || length == 0)
      return false;
...
private:
  ...
  __device__  __forceinline__ static uint64_t rotateLeft(uint64_t x, unsigned char bits)
  {
    return (x << bits) | (x >> (64 - bits));
  }

  /// process a single 64 bit value
  __device__ __forceinline__ static uint64_t processSingle(uint64_t previous, uint64_t input)
  {
    return rotateLeft(previous + input * Prime2, 31) * Prime1;
  }

  /// process a block of 4x4 bytes, this is the main part of the XXHash32 algorithm
  __device__ __forceinline__ static void process(const void* data, uint64_t& state0, uint64_t& state1, uint64_t& state2, uint64_t& state3)
  {
