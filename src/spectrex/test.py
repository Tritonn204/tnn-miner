...
for i in range(0, 4):
pre_pow_hash_np[i] = np.uint64(int(to_little(pre_pow_hash_g[i * 16: i * 16 + 16]), 16)) # Разбиение результата blake2b на 8 байтовые отрезки для представления в uint64  
...
bytes_header = np.concatenate((bytes_header, to_le_bytes(np.array([len(parent['parentHashes'])], dtype=np.uint64))))  
...
def header_serialization(header, for_pre_pow = True):
	...
	bytes_header = np.concatenate((bytes_header, decode_to_slice(header['hashMerkleRoot'])))


	# adding accepted_id_merkle_root
	bytes_header = np.concatenate((bytes_header, decode_to_slice(header['acceptedIdMerkleRoot'])))


	# adding uxto_commitment
	bytes_header = np.concatenate((bytes_header, decode_to_slice(header['utxoCommitment'])))


	# adding timestamp
	bytes_header = np.concatenate((bytes_header, to_le_bytes(np.array([timestamp], dtype=np.uint64))))


	# adding bits
	bytes_header = np.concatenate((bytes_header, to_le_bytes(np.array([header['bits']], dtype=np.uint32))))


	# adding nonce
	bytes_header = np.concatenate((bytes_header, to_le_bytes(np.array([nonce], dtype=np.uint64))))


	# adding daa_score
	bytes_header = np.concatenate((bytes_header, to_le_bytes(np.array([header['daaScore']], dtype=np.uint64))))


	# adding blue score
	bytes_header = np.concatenate((bytes_header, to_le_bytes(np.array([header['blueScore']], dtype=np.uint64))))
	...
	bytes_header = np.concatenate((bytes_header, to_le_bytes(np.array([blue_work_len], dtype=np.uint64))))
	bytes_header = np.concatenate((bytes_header, blue_work))


	# adding pruning_point
	bytes_header = np.concatenate((bytes_header, decode_to_slice(header['pruningPoint'])))
...
def main():
	...
	payload = {"hash": "17b59f5c71174ea5b6a796dce5ad64ad053769e895b53b8a3f31dabf2a7465e6"}
