...
char *xmrig::Platform::createUserAgent()
{
    ...
    int length = snprintf(buf, max, "%s/%s (Linux ", APP_NAME, STR2(APP_VERSION));

#   if defined(__x86_64__)
    length += snprintf(buf + length, max - length, "x86_64)");
#   elif defined(__aarch64__)
    length += snprintf(buf + length, max - length, "aarch64)");
#   elif defined(__arm__)
    length += snprintf(buf + length, max - length, "arm)");
#   else
    length += snprintf(buf + length, max - length, "i686)");
#   endif
