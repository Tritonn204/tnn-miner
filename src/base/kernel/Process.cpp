...
#       endif
    ...
    if (location == Process::ExeLocation) {
        ...
        const auto pos  = path.rfind(*TNN_DIR_SEPARATOR);
...
#   ifdef TNN_SHARED_DATADIR
    if (dataDir.empty()) {
        dataDir = fmt::format("{}" TNN_DIR_SEPARATOR ".xmrig" TNN_DIR_SEPARATOR, location(HomeLocation));
        MKDIR(dataDir);
...
xmrig::String xmrig::Process::location(Location location, const char *fileName)
{
    ...
    return fmt::format("{}" TNN_DIR_SEPARATOR "{}", path, fileName).c_str();
}
