...
#ifdef TNN_FEATURE_ENV
static std::map<String, String> variables;


static void createVariables()
{
    variables.insert({ "TNN_VERSION",  APP_VERSION });
    variables.insert({ "TNN_KIND",     APP_KIND });
    variables.insert({ "TNN_HOSTNAME", Env::hostname() });
    variables.insert({ "TNN_EXE",      Process::exepath() });
    variables.insert({ "TNN_EXE_DIR",  Process::location(Process::ExeLocation) });
    variables.insert({ "TNN_CWD",      Process::location(Process::CwdLocation) });
    variables.insert({ "TNN_HOME_DIR", Process::location(Process::HomeLocation) });
    variables.insert({ "TNN_TEMP_DIR", Process::location(Process::TempLocation) });
    variables.insert({ "TNN_DATA_DIR", Process::location(Process::DataLocation) });
