...
void jobThread(
  ...
  Nxl_HIP::copyDiff(ctx.cmpDiff);

  int h_nonceCount = 0;
  uint64_t h_nonceBuffer[Nxl_HIP::MAX_NONCES];
  ...
  while (localJobCounter == jobCounter)
  {
    ...
    Nxl_HIP::nonceCounter(ctx.d_nonceCount, &h_nonceCount, ctx.d_nonceBuffer, h_nonceBuffer);
    if (h_nonceCount > 0) {
...
void mineNexellia_hip()
{
  ...
  byte powHash[32];
  byte work[Nxl_HIP::INPUT_SIZE] = {0};
  byte devWork[Nxl_HIP::INPUT_SIZE] = {0};
...
waitForJob:
  ...
  while (!ABORT_MINER)
  {
    try
    {
      ...
      {
        byte *b2 = new byte[Nxl_HIP::INPUT_SIZE];
        switch (protocol)
        {
        case KAS_SOLO:
        ...
        memcpy(work, b2, Nxl_HIP::HASH_HEADER_SIZE);
        delete[] b2;
      ...
      if (devConnected && myJobDev.at("template").is_string())
      {
        ...
        {
          byte *b2d = new byte[Nxl_HIP::INPUT_SIZE];
          switch (protocol)
          {
          case KAS_SOLO:
          ...
          memcpy(devWork, b2d, Nxl_HIP::HASH_HEADER_SIZE);
          delete[] b2d;
