...
static constexpr double factor = 2.75;
...
// VEGA
  // Radeon VII
    archDim(HIP_ASTRIX_gfx906, 3840*4096*factor, 1024);
  // Vega 56/64
    archDim(HIP_ASTRIX_gfx900, 4090*4096*factor, 1024);

// RDNA1
  // RX 5600+
    archDim(HIP_ASTRIX_gfx1010, 2560*4096*factor, 1024);
  // Radeon Pro V520
    archDim(HIP_ASTRIX_gfx1011, 2560*4096*factor, 1024);
  // RX 5500+
    archDim(HIP_ASTRIX_gfx1012, 2560*4096*factor, 1024);
    archDim(HIP_ASTRIX_gfx1013, 1408*4096*factor, 1024);
// RDNA2
  // RX 6800+
    archDim(HIP_ASTRIX_gfx1030, 5120*4096*factor, 1024);
  // RX 6700+
    archDim(HIP_ASTRIX_gfx1031, 2560*4096*factor, 1024);
  // RX 6600+
    archDim(HIP_ASTRIX_gfx1032, 2048*4096*factor, 1024);
  // RX 6300+
    archDim(HIP_ASTRIX_gfx1034, 1024*4096*factor, 1024);

// RDNA 3
  // RX 7900+
    archDim(HIP_ASTRIX_gfx1100, 6114*4096*factor, 1024);
  // RX 7800+
    archDim(HIP_ASTRIX_gfx1101, 3840*4096*factor, 1024);
  // RX 7700+
    archDim(HIP_ASTRIX_gfx1102, 3456*4096*factor, 1024);
...
// Pascal
    archDim(HIP_ASTRIX_61, 3584*4096*factor, 1024);

// Volta
    archDim(HIP_ASTRIX_70, 5120*4096*factor, 1024);

// Turing
    archDim(HIP_ASTRIX_75, 4352*4096*factor, 1024);

// Ampere
    archDim(HIP_ASTRIX_86, 10496*4096*factor, 1024);

// Ada Lovelace (RTX 40 series)
    archDim(HIP_ASTRIX_89, 16384*4096*factor, 1024);

// Default nVidia architecture
    archDim(HIP_ASTRIX, 2048*4096*factor, 1024);
...
static inline const std::unordered_map<std::string, ArchDims> archDimsMap = {
  ARCH_DIM_ENTRY(HIP_ASTRIX, gfx900),   // Vega 56/64
  ARCH_DIM_ENTRY(HIP_ASTRIX, gfx906),   // Radeon VII

  ARCH_DIM_ENTRY(HIP_ASTRIX, gfx1010),  // RX 5600+
  ARCH_DIM_ENTRY(HIP_ASTRIX, gfx1011),  // Radeon Pro V520
  ARCH_DIM_ENTRY(HIP_ASTRIX, gfx1012),  // RX 5500+
  ARCH_DIM_ENTRY(HIP_ASTRIX, gfx1013),  // RX 5300

  ARCH_DIM_ENTRY(HIP_ASTRIX, gfx1030),  // RX 6800+
  ARCH_DIM_ENTRY(HIP_ASTRIX, gfx1031),  // RX 6700+
  ARCH_DIM_ENTRY(HIP_ASTRIX, gfx1032),  // RX 6600+
  ARCH_DIM_ENTRY(HIP_ASTRIX, gfx1034),  // RX 6300+

  ARCH_DIM_ENTRY(HIP_ASTRIX, gfx1100),  // RX 7900+
  ARCH_DIM_ENTRY(HIP_ASTRIX, gfx1101),  // RX 7800+
  ARCH_DIM_ENTRY(HIP_ASTRIX, gfx1102),  // RX 7700+
};
#elif defined(__HIP_PLATFORM_NVIDIA__)
static inline const std::unordered_map<std::string, ArchDims> archDimsMap = {
  ARCH_DIM_ENTRY(HIP_ASTRIX, 61),
  ARCH_DIM_ENTRY(HIP_ASTRIX, 70),
  ARCH_DIM_ENTRY(HIP_ASTRIX, 75),
  ARCH_DIM_ENTRY(HIP_ASTRIX, 86),
  ARCH_DIM_ENTRY(HIP_ASTRIX, 89),
};
#endif
