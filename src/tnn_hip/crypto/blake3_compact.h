...
__device__ void blake3_hasher_update_aix(blake3_hasher* self, const void* input)
{
...
__device__ void blake3_hasher_finalize_seek_aix(
    const blake3_hasher* self, uint64_t seek, uint8_t* out)
{
  if (self->cv_stack_len == 0) {
      output_t output = chunk_state_output(&self->chunk);
      output_root_bytes(&output, seek, out, 32);
      return;
  }
  output_t output;
...
__device__ void blake3_hasher_finalize_aix(const blake3_hasher* self, uint8_t* out)
{
    blake3_hasher_finalize_seek_aix(self, 0, out);
}
