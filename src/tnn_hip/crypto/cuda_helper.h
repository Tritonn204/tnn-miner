...
#ifdef __CUDA_ARCH__
DEV_INLINE uint32_t hip_swab32(const uint32_t x)
{
    /* device */
...
#ifdef __CUDA_ARCH__
DEV_INLINE uint64_t hip_swab64(const uint64_t x)
{
    uint64_t result;
    uint2 t;
    asm("mov.b64 {%0,%1},%2; \n\t" : "=r"(t.x), "=r"(t.y) : "l"(x));
...
{
    return MAKE_ULONGLONG(hip_swab32(v.y), hip_swab32(v.x));
}
static DEV_INLINE uint2 vectorizeswap(uint64_t v)
{
    uint2 result;
    LOHI(result.y, result.x, v);
    result.x = hip_swab32(result.x);
    result.y = hip_swab32(result.y);
    return result;
}
