...
// Experimental compile-only port. Existing Iris defaults are not changed.
#include "api.hpp"
    ...
    for(unsigned kk=0; kk<k; kk+=32) {
        ...
        *reinterpret_cast<I4*>(as+aw)=ga;
        ...
        if constexpr(!Raw) if((kk+32)%Rank==0)
            Pearl::checkpoint(c,transcript,kk);
...
hipError_t launch_paired(const int8_t* a,const int8_t* b,int32_t* d,
