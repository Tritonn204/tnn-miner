...
// Mirrors the studied Tensile MT64x64x32, SS1, WG32_4_1 ownership.
// A LDS: 64 contiguous rows per K. B LDS: 32 contiguous K per column,
...
__device__ __forceinline__ unsigned b_offset(unsigned n, unsigned k) {
    unsigned x = n * 32 + k;
    if constexpr(Swizzle) return x ^ ((n&28u)<<2);
    else return Gfx1100Paired::b_offset(n,k);
}

template<int NB> struct Operands { I4 a0, a1, b[NB]; };
template<class Schedule,int KS,int NB,bool Paired=false,bool Swizzle=false,bool Half=false>
__device__ __forceinline__ Operands<NB> operands(const unsigned char* a,
    ...
    if constexpr(Half) {
        ...
        unsigned addr=unsigned(reinterpret_cast<uintptr_t>(a))+(am&~3u)+KS*Schedule::tile_m;
        asm volatile(
            "ds_read2_b32 %0, %8 offset0:%c9 offset1:%c10\n\t"
            ...
            "s_waitcnt lgkmcnt(0)"
            : "=&v"(p0),"=&v"(p1),"=&v"(p2),"=&v"(p3),"=&v"(p4),"=&v"(p5),"=&v"(p6),"=&v"(p7)
            : "v"(addr),"n"(0*Schedule::tile_m/4),"n"(1*Schedule::tile_m/4),"n"(2*Schedule::tile_m/4),"n"(3*Schedule::tile_m/4),"n"(4*Schedule::tile_m/4),"n"(5*Schedule::tile_m/4),"n"(6*Schedule::tile_m/4),"n"(7*Schedule::tile_m/4),"n"((8-(Schedule::tile_m==128?8:0))*Schedule::tile_m/4),"n"((9-(Schedule::tile_m==128?8:0))*Schedule::tile_m/4),"n"((10-(Schedule::tile_m==128?8:0))*Schedule::tile_m/4),"n"((11-(Schedule::tile_m==128?8:0))*Schedule::tile_m/4),"n"((12-(Schedule::tile_m==128?8:0))*Schedule::tile_m/4),"n"((13-(Schedule::tile_m==128?8:0))*Schedule::tile_m/4),"n"((14-(Schedule::tile_m==128?8:0))*Schedule::tile_m/4),"n"((15-(Schedule::tile_m==128?8:0))*Schedule::tile_m/4),"v"(addr+(Schedule::tile_m==128?8*Schedule::tile_m:0)) : "memory");
    ...
    #pragma unroll
    for(int i=0;i<NB;++i) x.b[i] = *reinterpret_cast<const I4*>(b+b_offset<Swizzle>(bn+i*32,KS));
    return x;
}
template<int NB,bool Priority>
__device__ __forceinline__ void compute(I8 (&c)[2*NB], const Operands<NB>& x) {
    ...
    if constexpr(Priority) asm volatile("s_setprio 3");
    #pragma unroll
    for(int i=0;i<NB;++i) {
        if constexpr(Priority) {
            asm volatile("v_wmma_i32_16x16x16_iu8 %0, %1, %2, %0 neg_lo:[1,1,0]" : "+v"(c[2*i]) : "v"(x.b[i]),"v"(x.a0));
            asm volatile("v_wmma_i32_16x16x16_iu8 %0, %1, %2, %0 neg_lo:[1,1,0]" : "+v"(c[2*i+1]) : "v"(x.b[i]),"v"(x.a1));
        } else {
        c[2*i] = __builtin_amdgcn_wmma_i32_16x16x16_iu8_w32(true,x.b[i],true,x.a0,c[2*i],false);
        c[2*i+1] = __builtin_amdgcn_wmma_i32_16x16x16_iu8_w32(true,x.b[i],true,x.a1,c[2*i+1],false);
        }
    }
    if constexpr(Priority) asm volatile("s_setprio 0");
}

template<bool Local>
__device__ __forceinline__ __attribute__((convergent)) void shared_barrier() {
    ...
    if constexpr(Local) asm volatile("s_waitcnt lgkmcnt(0)\n\ts_barrier" ::: "memory");
    else __syncthreads();
}
