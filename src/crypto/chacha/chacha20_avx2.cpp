...
static void ChaCha20EncryptXelis_avx2_impl(
    ...
    int rounds)
{
    ...
    for (size_t iter = 0; iter < iterations; iter++)
    {
        ...
        for (int i = rounds; i > 0; i -= 2)
        {
            ...
            QR_AVX2(x0, x4, x8,  x12); QR_AVX2(x1, x5, x9,  x13);
            QR_AVX2(x2, x6, x10, x14); QR_AVX2(x3, x7, x11, x15);
            QR_AVX2(x0, x5, x10, x15); QR_AVX2(x1, x6, x11, x12);
            QR_AVX2(x2, x7, x8,  x13); QR_AVX2(x3, x4, x9,  x14);
        ...
        // Transpose and store
        #define TRANSPOSE_STORE_AVX2(xa, xb, xc, xd, off0, off1) \
        { \
        ...
        TRANSPOSE_STORE_AVX2(x0,  x1,  x2,  x3,  0,  64);
        TRANSPOSE_STORE_AVX2(x4,  x5,  x6,  x7,  16, 80);
        TRANSPOSE_STORE_AVX2(x8,  x9,  x10, x11, 32, 96);
        TRANSPOSE_STORE_AVX2(x12, x13, x14, x15, 48, 112);
        
        #undef TRANSPOSE_STORE_AVX2
...
void ChaCha20EncryptXelis(
    ...
    int rounds)
{
    ChaCha20EncryptXelis_avx2_impl<StorePolicy::TEMPORAL>(
...
void ChaCha20EncryptXelisNT(
    ...
    int rounds)
{
    ChaCha20EncryptXelis_avx2_impl<StorePolicy::NON_TEMPORAL>(
        keys, nonces, outputs, bytes_per_stream, rounds);
}
#endif