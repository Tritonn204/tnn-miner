...
namespace WalaHash
{
  ...
  static inline void heavyHash(byte *scratch, const matrix &mat, byte *out)
  {
    ...
    for (int i = 0; i < QUARTER_MATRIX_SIZE; i++)
    {
      ...
      sum1 = (sum1 & 0xF) ^ ((sum1 >> 4) & 0xF) ^ ((sum1 >> 8) & 0xF);
      sum2 = (sum2 & 0xF) ^ ((sum2 >> 4) & 0xF) ^ ((sum2 >> 8) & 0xF);
      sum3 = (sum3 & 0xF) ^ ((sum3 >> 4) & 0xF) ^ ((sum3 >> 8) & 0xF);
      sum4 = (sum4 & 0xF) ^ ((sum4 >> 4) & 0xF) ^ ((sum4 >> 8) & 0xF);

      scratch[i * 2] ^= ((uint8_t)sum1 << 4) | (uint8_t)sum2;
      scratch[i * 2 + 1] ^= ((uint8_t)sum3 << 4) | (uint8_t)sum4;
    }

    blake3(scratch, 32, scratch);
  }

  static inline void sha3_256_wala(byte *in, byte *scratch) {
    scratch[32] = 0x06;
    scratch[135] = 0x80;
  ...
  void hash(worker &worker, byte *in, int len, byte *out)
  {
    ...
    sha3_256_wala(scratchData, scratchData);
  ...
  int test() {
    ...
    const char* expected = "de56d588edfd638c448993310ded7575b3deb6c7a3473697b5c751c2715fb048";
  ...
  // int test() {
    ...
    // alignas(64) WalaHash::worker *w = (WalaHash::worker *)malloc_huge_pages(sizeof(WalaHash::worker));
