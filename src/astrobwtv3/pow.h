...
public:
  Constructor(const T *const _data, suffix *const _P, const t_index _N, const t_index _K, const t_index reserved)
      : data(_data), P(_P), R(reinterpret_cast<t_index *>(_P + _N)), RE(R + 1), R2(reserved >= _K * 2 ? R + reserved - _K + 1 : NULL), N(_N), K(_K), n1(0), d1(0), name(0)
  {
    ...
    if (R2)
    {
      assert(R2 >= R + K + 1);
      makeBuckets();
      std::cout << (K - 1) * sizeof(t_index) << std::endl;
...
inline void hashSHA256(SHA256_CTX &sha256, const char *input, unsigned char *digest, unsigned long inputSize)
{
  SHA256_Init(&sha256);
