...
waitForJob:
  ...
  while (true)
  {
    try
    {
      ...
      {
        byte *b2 = new byte[HooHash::INPUT_SIZE];
        switch (protocol)
        {
        case KAS_SOLO:
        ...
        memcpy(work, b2, HooHash::INPUT_SIZE);
        HooHash::newMatrix(work, worker->matBuffer, *worker);
        // HooHash::genPrePowHash(b2, *worker);/
        // HooHash::newMatrix(b2, worker->mat);
        delete[] b2;
      ...
      if (devConnected && myJobDev.at("template").is_string())
      {
        ...
        {
          byte *b2d = new byte[HooHash::INPUT_SIZE];
          switch (protocol)
          {
          case KAS_SOLO:
          ...
          memcpy(devWork, b2d, HooHash::INPUT_SIZE);
          HooHash::newMatrix(devWork, devWorker->matBuffer, *devWorker);
          // HooHash::genPrePowHash(b2d, *devWorker);
          // HooHash::newMatrix(b2d, devWorker->mat);
          delete[] b2d;
      ...
      while (localJobCounter == jobCounter)
      {
        ...
        // printf("after nonce: %s\n", hexStr(WORK, HooHash::INPUT_SIZE).c_str());
        ...
        HooHash::worker &usedWorker = devMine ? *devWorker : *worker;
        HooHash::hash(usedWorker, WORK, HooHash::INPUT_SIZE, powHash);
        ...
        if (HooHash::checkNonce(((uint64_t*)usedWorker.scratchData),((uint64_t*)cmpDiff)))
        {
          // printf("cmpDiff:\n");
          // for(int i = 0; i < 32; i++) {
        ...
        //   std::string b64 = base64::to_base64(std::string((char *)&WORK[0], XELIS_TEMPLATE_SIZE));
          ...
          else
          {
            ...
            case KAS_STRATUM:
              ...
              // std::cout << "blob: " << hexStr(&WORK[0], HooHash::INPUT_SIZE).c_str() << std::endl;
              // std::cout << "nonce: " << n << std::endl;
              // std::cout << "extraNonce: " << hexStr(&WORK[HooHash::INPUT_SIZE - 48], enLen).c_str() << std::endl;
