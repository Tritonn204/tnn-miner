...
void mineWaglayla(int tid)
{
  ...
  byte powHash[32];
  byte work[WalaHash::INPUT_SIZE] = {0};
  byte devWork[WalaHash::INPUT_SIZE] = {0};
  ...
  WalaHash::worker *worker = (WalaHash::worker *)malloc(sizeof(WalaHash::worker));
  WalaHash::worker *devWorker = (WalaHash::worker *)malloc(sizeof(WalaHash::worker));
...
waitForJob:
  ...
  while (!ABORT_MINER)
  {
    try
    {
      ...
      {
        byte *b2 = new byte[WalaHash::INPUT_SIZE];
        switch (protocol)
        {
        case KAS_SOLO:
        ...
        memcpy(work, b2, WalaHash::INPUT_SIZE);
        WalaHash::newMatrix(work, worker->matBuffer, *worker);
        // WalaHash::genPrePowHash(b2, *worker);/
        // WalaHash::newMatrix(b2, worker->mat);
        delete[] b2;
      ...
      if (devConnected && myJobDev.at("template").is_string())
      {
        ...
        {
          byte *b2d = new byte[WalaHash::INPUT_SIZE];
          switch (protocol)
          {
          case KAS_SOLO:
          ...
          memcpy(devWork, b2d, WalaHash::INPUT_SIZE);
          WalaHash::newMatrix(devWork, devWorker->matBuffer, *devWorker);
          // WalaHash::genPrePowHash(b2d, *devWorker);
          // WalaHash::newMatrix(b2d, devWorker->mat);
          delete[] b2d;
      ...
      while (localJobCounter == jobCounter)
      {
        ...
        // printf("after nonce: %s\n", hexStr(WORK, WalaHash::INPUT_SIZE).c_str());
        ...
        WalaHash::worker &usedWorker = devMine ? *devWorker : *worker;
        WalaHash::hash(usedWorker, WORK, WalaHash::INPUT_SIZE, powHash);
        ...
        if (WalaHash::checkNonce(((uint64_t*)usedWorker.scratchData),((uint64_t*)cmpDiff)))
        {
          // printf("cmpDiff:\n");
          // for(int i = 0; i < 32; i++) {
        ...
        //   std::string b64 = base64::to_base64(std::string((char *)&WORK[0], XELIS_TEMPLATE_SIZE));
          ...
          if (devMine)
          {
            ...
            {
            case KAS_SOLO:
              devShare = {{"block_template", hexStr(&WORK[0], WalaHash::INPUT_SIZE).c_str()}};
              break;
            case KAS_STRATUM:
          ...
          else
          {
            ...
            {
            case KAS_SOLO:
              share = {{"block_template", hexStr(&WORK[0], WalaHash::INPUT_SIZE).c_str()}};
              break;
            case KAS_STRATUM:
              std::vector<char> nonceStr;
              // Num(std::to_string((n << enLen*8) >> enLen*8).c_str(),10).print(nonceStr, 16);
