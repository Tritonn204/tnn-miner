...
# endif
  return 0;
#else
// Initialize critical section or return error code directly
# if defined(UNDER_CE)
::InitializeCriticalSection(&crit_section_);
// Assuming success as InitializeCriticalSection does not return a value
# elif defined(BOOST_ASIO_WINDOWS_APP)
if (!::InitializeCriticalSectionEx(&crit_section_, 0, 0))
  return ::GetLastError();
# else
...
// Assuming the only critical failure we're checking for is out of memory,
// which is checked immediately after the attempt to initialize the critical section.
