...
# endif
#else
// No SEH, use return value checking for error handling
# if defined(UNDER_CE)
::InitializeCriticalSection(&crit_section_);
// Note: Assuming InitializeCriticalSection does not fail in UNDER_CE,
// otherwise you need a similar check as below.
...
// Since the original SEH __except was specifically handling STATUS_NO_MEMORY,
// you need to ensure above methods can report such an error and handle accordingly.
// If GetLastError() returns ERROR_OUTOFMEMORY, it's already handled by checking the return values.
#endif
